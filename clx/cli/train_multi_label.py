...
def get_label_predictions(data_dir, data, label):
    ...
    if len(needs_preds) > 0:
        needs_preds["prediction"] = label.pipe.predict(
            needs_preds["text"].tolist(),
            num_workers=32,
        )
        needs_preds = needs_preds[["document_id", "prediction"]]
...
def train_multi_label(project_name):
    ...
    for label in tqdm(project.labels.all(), desc="Getting label predictions"):
        if label.finetune_status == "completed":
            ...
            label_doc_ids = label_data[label_data["prediction"] == "yes"][
                "document_id"
            ]
            data[str(label.id)] = data["document_id"].isin(label_doc_ids)

    data["labels"] = data[label_ids].apply(
        lambda x: [lid for lid in label_ids if x[lid]], axis=1
    )
    ...
    train_run = training_run(
        ...
        run_name=f"multi-label-{project.id}",
    ...
    train_run.train(train_data, eval_data, overwrite=True, remote=True)
